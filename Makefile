...
lint: ## check style with flake8 and isort
	flake8 data_segmentor tests
	isort -c --recursive data_segmentor tests
...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find data_segmentor -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive data_segmentor
	isort --apply --atomic --recursive data_segmentor
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source data_segmentor -m pytest
	coverage report -m
	coverage html
...
docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc --module-first --separate -o docs/api/ data_segmentor
	$(MAKE) -C docs html

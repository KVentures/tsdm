...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find tsdm -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive tsdm
	isort --apply --atomic --recursive tsdm
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source tsdm -m pytest
	coverage report -m
	coverage html
...
docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc --module-first --separate -o docs/api/ tsdm
	$(MAKE) -C docs html

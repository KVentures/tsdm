...
Time Series Data Manager could always use more documentation, whether as part of the
official Time Series Data Manager docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `Time Series Data Manager` for local development.

1. Fork the `Time Series Data Manager` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/tsdm.git
...
2. The tests that cover a module called ``tsdm/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/tsdm/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_tsdm
...
2. Update the version in ``setup.cfg``, ``tsdm/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``tsdm/__init__.py``
...
.. _GitHub issues page: https://github.com/HDI-Project/tsdm/issues
.. _Travis Build Status page: https://travis-ci.org/HDI-Project/tsdm/pull_requests

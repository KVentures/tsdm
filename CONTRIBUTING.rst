...
Data Segmentor could always use more documentation, whether as part of the
official Data Segmentor docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `Data Segmentor` for local development.

1. Fork the `Data Segmentor` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/data-segmentor.git

3. Install your local copy into a virtualenv. Assuming you have virtualenvwrapper installed,
   this is how you set up your fork for local development::

    $ mkvirtualenv data-segmentor
    $ cd data-segmentor/
...
2. The tests that cover a module called ``data_segmentor/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/data_segmentor/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_data_segmentor
...
2. Update the version in ``setup.cfg``, ``data_segmentor/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``data_segmentor/__init__.py``
...
.. _GitHub issues page: https://github.com/HDI-Project/data-segmentor/issues
.. _Travis Build Status page: https://travis-ci.org/HDI-Project/data-segmentor/pull_requests
